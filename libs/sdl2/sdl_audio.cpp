...
namespace audio
{
    ...
    constexpr int MAX_SOUND_TRACKS = 8;
...
namespace internal
{
    ...
    static f32 get_music_volume()
    {
        ...
        auto i_volume = Mix_VolumeMusic(QUERY);        

        return (f32)(i_volume - MIN) / (MAX - MIN);
    }


    static f32 get_sound_channel_volume(SoundId sound_id)
    {
        constexpr int MAX = MIX_MAX_VOLUME;
        constexpr int MIN = 0;
        constexpr int QUERY = -1;

        auto track_channel = sound_id.value;

        auto i_volume = Mix_Volume(track_channel, QUERY);

        return (f32)(i_volume - MIN) / (MAX - MIN);
    }


    static f32 set_sound_channel_volume(SoundId sound_id, f32 volume)
    {
        constexpr int MAX = MIX_MAX_VOLUME;
        constexpr int MIN = 0;
        constexpr int QUERY = -1;
        ...
        auto i_volume = math::cxpr::round_to_signed<int>(volume * (MAX - MIN));
        if (i_volume != Mix_Volume(track_channel, QUERY))
        {
            Mix_Volume(track_channel, i_volume);
        }
        
        return get_sound_channel_volume(sound_id);
    }  
    ...
    {
        SoundId all = { -1 };
        return set_sound_channel_volume(all, volume);
    }
    ...
    static void set_sound_id(Sound& sound, sound_p data)
    {
        ...
        sound.id.value = n_sound_tracks % MAX_SOUND_TRACKS;
    }


    static TrackChannel get_track_channel(Sound const& sound)
    {
        TrackChannel tc{};

        tc.value = sound.id.value < 0 ? -1 : sound.id.value;
    ...
    {
        audio_assert(sound.handle && " *** no sound data *** ");
        
        constexpr int N_REPEATS = 0;

        auto track_channel = get_track_channel(sound);

        track_channel.value = Mix_PlayChannel(track_channel.value, (sound_p)sound.handle, N_REPEATS);
        if (sound.id.value < 0)
        {
            sound.id.value = track_channel.value;
            sound_tracks[track_channel.value] = &sound;
        }
    ...
    static void play_sound_track_loop(Sound& sound)
    {
        ...
        track_channel.value = Mix_PlayChannel(track_channel.value, (sound_p)sound.handle, FOREVER);
        if (sound.id.value < 0)
        {
            sound.id.value = track_channel.value;
            sound_tracks[track_channel.value] = &sound;
    ...
    static void set_music_id(Music& music, music_p data)
    {
        ...
        music.id.value = n_music_tracks++;
    }


    static bool is_current_music_track(Music const& music)
    {
        return music_track && music_track->id.value == music.id.value;
    }
    ...
    {
        audio_assert(music.handle && " *** no music data *** ");

        constexpr int FOREVER = -1;
        auto err = Mix_PlayMusic((music_p)music.handle, FOREVER);
    ...
    static void fade_in_music_track(Music& music, u32 fade_ms)
    {
        ...
        constexpr int FOREVER = -1;
        auto err = Mix_FadeInMusic((music_p)music.handle, FOREVER, (int)fade_ms);
...
namespace audio
{
    void destroy_music(Music& music)
    {
        if (music.handle)
        {
            ...
            Mix_FreeMusic((music_p)music.handle);
        }

        if (internal::is_current_music_track(music))
        {
            music_track = 0;
        }
    ...
    void destroy_sound(Sound& sound)
    {
        if (sound.handle)
        {
            ...
            Mix_FreeChunk((sound_p)sound.handle);
        }        
    ...
    bool init_audio()
    {
        ...
        if (rc < 0)
        {
            sdl::print_error("SDL_Init");
            return false;
        }

        rc = Mix_Init(MIX_INIT_MP3 | MIX_INIT_OGG);
        if (rc < 0)
        {
        ...
        rc = Mix_OpenAudio(freq, format, channels, chunk_size);
        if (rc < 0)
        {
    ...
    bool load_music_from_file(cstr music_file_path, Music& music)
    {
        ...
        internal::set_music_id(music, data);
    ...
    bool load_sound_from_file(cstr sound_file_path, Sound& sound)
    {
        ...
        internal::set_sound_id(sound, data);
    ...
    f32 set_music_volume(f32 volume)
    {
        ...
        return internal::get_music_volume();
    }
    ...
    f32 set_sound_volume(f32 volume)
    {
        ...
        return internal::set_sound_volume_all_channels(volume);
    }
    ...
    f32 set_sound_volume(Sound& sound, f32 volume)
    {
        ...
        return internal::set_sound_channel_volume(sound.id, volume);
    }
    ...
    void play_music(Music& music)
    {
        ...
        internal::play_music_track(music);
    }
    ...
    void stop_music()
    {
        internal::stop_music_track();
    }


    void fade_in_music(Music& music, u32 fade_ms)
    {
        internal::fade_in_music_track(music, fade_ms);
    }


    void fade_out_music(u32 fade_ms)
    {
        internal::fade_out_music_track(fade_ms);
    }
    ...
    void play_sound(Sound& sound)
    {
        ...
        internal::play_sound_track_once(sound);
    }
    ...
    void play_sound_loop(Sound& sound)
    {
        ...
        internal::play_sound_track_loop(sound);
    }
    ...
    void stop_sound(Sound& sound)
    {
        ...
        Mix_HaltChannel(sound.id.value);
        sound.is_on = false;
    }
